...
numTrials        = 50;
load([TempDatDir 'Modeled_6s_AAV_fix.mat']);
s2cDataSet = nDataSet;
...
numTrials        = 50;
load([TempDatDir 'Modeled_GP43_fix.mat']);
s2cDataSet = nDataSet;
s2cFiringRates   = generateDPCADataLinearData(s2cDataSet, numTrials);
s2cFiringRatesMean   = squeeze(mean(s2cFiringRates, 4));
s2cFiringRatesMean   = [squeeze(s2cFiringRatesMean(:, 1, :)), squeeze(s2cFiringRatesMean(:, 2, :))];
...
numTrials        = 50;
load([TempDatDir 'Modeled_GP517_fix.mat']);
s2cDataSet = nDataSet;
s2cFiringRates   = generateDPCADataLinearData(s2cDataSet, numTrials);
s2cFiringRatesMean   = squeeze(mean(s2cFiringRates, 4));
% s2cFiringRatesMean   = [squeeze(s2cFiringRatesMean(:, 1, :)), squeeze(s2cFiringRatesMean(:, 2, :))];
...
for nUnit = 1:size(s2cFiringRatesMean, 1)
    ...
    nlParams(3, nUnit, :) = param;
end

save([TempDatDir 'FineTunedNLParamsFix.mat'], 'nlParams');