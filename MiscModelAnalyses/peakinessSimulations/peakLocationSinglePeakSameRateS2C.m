...
for nCell     = 1:length(peakOnlyData)
    ...
    if time_  <= numTimeBin
        ...
        spk_time  = cell(numYes, 1);
        for ntrial = 1:numYes
            t_ = (rand(n_spk, 1)-0.5)*2*binsize;
            spk_time{ntrial} = peak_time+t_;
        end
        peakOnlyData(nCell).unit_yes_trial = zeros(numYes, numTimeBin);
        peakOnlyData(nCell).unit_yes_trial(:, time_) = n_spk;
        ...
        peak_time = timeBins(time_);
        spk_time  = cell(numNo, 1);
        for ntrial = 1:numNo
            t_ = (rand(n_spk, 1)-0.5)*2*binsize;
            spk_time{ntrial} = peak_time+t_;
        end
        peakOnlyData(nCell).unit_no_trial = zeros(numNo, numTimeBin);
        peakOnlyData(nCell).unit_no_trial(:, time_) = n_spk;
...
box off
legend('Ephys', 'S2C')
setPrint(8, 6, 'peakLocationSingleSameRatePeakS2C', 'pdf')
...
xlabel('Time')
hold off
setPrint(8, 6, 'peakLocationSinglePeakSameRatesS2C_distrs', 'pdf')