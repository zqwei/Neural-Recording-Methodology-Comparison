...
load([TempDatDir 'Shuffle_Spikes_Hi_intra_Short_Delay.mat'], 'nDataSet');
...
nData                      = 1;
DataSetList(nData).name    = 'Modeled_6s_AAV_intra';
...
nDataSet               = getFakeCaImagingDataSigmoid(spikeDataSet, params);
nData                      = 2;
DataSetList(nData).name    = 'Modeled_GP43_intra';
DataSetList(nData).params  = params; 
DataSetList(nData).ActiveNeuronIndex = ActiveNeuronIndex;
save([TempDatDir DataSetList(nData).name '.mat'], 'nDataSet'); 

save([TempDatDir 'DataListS2CIntraModel.mat'], 'DataSetList');